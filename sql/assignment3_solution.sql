...
AS 
BEGIN 
    SELECT @TotalOrderQty = SUM(Quantity)  
    FROM [Order Details] OD 
    JOIN Products P ON P.ProductID = OD.ProductID 
...
DECLARE @Tot INT 
EXEC sp_Product_Order_Quantity_Gaddam 11,@Tot OUT PRINT @Tot 
--3 
ALTER PROC sp_Product_Order_City_Gaddam @ProductName NVARCHAR(50) 
AS
BEGIN 
...
--4 
CREATE TABLE People_Gaddam ( id int , name nvarchar(100), city int )
create table City_Gaddam ( id int, city nvarchar(100) ) 
BEGIN
    TRAN  
...
if exists(  
            select id from People_Gaddam 
            where city = (select id from City_Gaddam where city = 'Seatle')) 
            begin 
                insert into City_Gaddam values(3,'Madison')
                update People_Gaddam set city = 'Madison' 
...
select * from Packers_Gaddam commit 
...
SELECT * FROM Customers EXCEPT SELECT * FROM Customers
